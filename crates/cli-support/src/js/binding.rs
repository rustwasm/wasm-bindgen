...
impl<'a, 'b> Builder<'a, 'b> {
    ...
    pub fn process(
        ...
        args_data: &Option<Vec<FunctionArgumentData>>,
        asyncness: bool,
        variadic: bool,
        generate_jsdoc: bool,
        debug_name: &str,
        ...
        let args_names = function_args
            .iter()
            .map(|v| v.name.as_str())
            .collect::<Vec<&str>>();
        code.push('(');
        if variadic {
            if let Some((last, non_variadic_args)) = args_names.split_last() {
    ...
    fn typescript_signature(
        &self,
        args_data: &[FunctionArgumentData],
        arg_tys: &[&AdapterType],
    ...
    fn ts_doc_comments(
        ...
        for FunctionArgumentData { name, desc, .. } in args_data.iter() {
            let mut arg = "@param ".to_string();
            arg.push_str(name);

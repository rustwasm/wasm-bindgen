...
const wbg_holdsnumber_method_with_attr: (a: number, b?: boolean) => number = wbg.HoldsNumber.static_fn_with_attr(1).method_with_attr;

test("async function fn_with_attr", async () => {
  let result = await wbg.fn_with_attr(4);
...
test("HoldsNumber methods", () => {
  const num1 = wbg.HoldsNumber.static_fn_with_attr(4).inner;
  const num2 = wbg.HoldsNumber.static_fn_with_attr(3, 4).inner;
  expect(num1).toEqual(num2);

  const holdsNumber = wbg.HoldsNumber.static_fn_with_attr(8);
  let result = holdsNumber.method_with_attr(4);

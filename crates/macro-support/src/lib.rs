...
pub fn expand_class_marker(
    ...
    let mut tokens = proc_macro2::TokenStream::new();
    tokens.append_all(item.attrs.iter().filter(|attr| matches!(attr.style, syn::AttrStyle::Outer)));
    ...
    item.block.brace_token.surround(&mut tokens, |tokens| {
        ...
        tokens.append_all(item.attrs.iter().filter(|attr| matches!(attr.style, syn::AttrStyle::Inner(_))));

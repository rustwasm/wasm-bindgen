...
impl<'a> MacroParse<(TokenStream, &'a mut TokenStream)> for syn::Item {
    fn macro_parse(
        self,
        program: &mut ast::Program,
        (args, tokens): (TokenStream, &'a mut TokenStream),
    ) -> Result<(), Diagnostic> {
        // Initial parsing of attribute args. This does syntax validation, but does not do any checking
        // of field names or values; that depends on the variant of `self`.
        let args = match meta::Meta::<Rhs>::with_body(parse_quote!(wasm_bindgen), args)?.into() {
            syn::Meta::List(list) => list.nested.into_iter().collect(),
            syn::Meta::Path(_) => vec![],
            syn::Meta::NameValue(nv) => {
                return Err(Diagnostic::from(
                    darling::Error::unsupported_format("name-value").with_span(&nv),
                ))
            }
        };

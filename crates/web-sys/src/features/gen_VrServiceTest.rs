...
extern "C" {
    ...
    #[doc = "*This API requires the following crate features to be activated: `VrServiceTest`*"]
    #[doc = ""]
    #[doc = "Return value: While the Promise can produce any JsValue as far as the type system is concerned, practically it is expected to contain a `VrMockController`. It can be converted like `let result: VrMockController = result?.await.into();`."]
    ...
    #[doc = "*This API requires the following crate features to be activated: `VrServiceTest`*"]
    #[doc = ""]
    #[doc = "Return value: While the Promise can produce any JsValue as far as the type system is concerned, practically it is expected to contain a `VrMockDisplay`. It can be converted like `let result: VrMockDisplay = result?.await.into();`."]
